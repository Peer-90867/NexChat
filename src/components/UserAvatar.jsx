export const UserAvatar = ({ user, size = "md", isOnline = false, status = 'online' }) => {
  const sizeClasses = {
    sm: "w-8 h-8 text-xs",
  ...
  const currentStatusColor = statusColors[status] || 'bg-green-500';
  ...
  return (
    <div className="relative inline-block">
      ...
      {isOnline && (
        <span className={`absolute bottom-0 right-0 block w-3 h-3 rounded-full ${currentStatusColor} ring-2 ring-[#111111]`} />
      )}
    </div>
  );
};
